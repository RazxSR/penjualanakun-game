...
class PembayaranController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				pembayaran.idpembayaran LIKE ? OR 
				pembayaran.tgl_pembayaran LIKE ? OR 
				pembayaran.jmlh_uang LIKE ? OR 
				pembayaran.status LIKE ?
			)";
			$search_params = array(
				"%$text%","%$text%","%$text%","%$text%"
			);
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("pembayaran.idpembayaran", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		$tablename = $this->tablename;
		$fields = array("pembayaran.idpembayaran", 
			"pembayaran.tgl_pembayaran", 
			"pembayaran.jmlh_uang", 
			"pembayaran.status", 
		...
		if($value){
			...
			$db->where("pembayaran.idpembayaran", $rec_id);; //select record based on primary key
		}
		$db->join("product", "pembayaran.idpembayaran = product.idproduct", "INNER ");  
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("tgl_pembayaran","jmlh_uang","status");
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'tgl_pembayaran' => 'required',
				'jmlh_uang' => 'required|numeric',
				'status' => 'required',
			);
			$this->sanitize_array = array(
				'tgl_pembayaran' => 'sanitize_string',
				'jmlh_uang' => 'sanitize_string',
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("idpembayaran","tgl_pembayaran","jmlh_uang","status");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'tgl_pembayaran' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("pembayaran.idpembayaran", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("pembayaran.idpembayaran", $arr_rec_id, "in");
